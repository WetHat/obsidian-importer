...
> 
> There is no `switch` statement in Python. You can use an `if..elif..else` statement to do the same thing (and in some cases, use a [dictionary](Data%20Structures.md#^dictionary) to do it quickly)

## The while Statement

The `while` statement allows you to repeatedly execute a block of statements as long as a condition is true. A `while` statement is an example of what is called a _looping_ statement. A `while` statement can have an optional `else` clause.
...
The `for..in` statement is another looping statement which _iterates_ over a sequence of objects i.e. go through each item in a sequence. We will see more about [sequences](Data%20Structures.md#^sequence) in detail in later chapters. What you need to know right now is that a sequence is just an ordered collection of items.
...
Note that `range()` generates only one number at a time, if you want the full list of numbers, call `list()` on the `range()`, for example, `list(range(5))` will result in `[0, 1, 2, 3, 4]`. Lists are explained in the [data structures chapter](Data%20Structures.md#^data-structures).
...
Remember that the `else` part is optional. When included, it is always executed once after the `for` loop is over unless a [break](Control%20flow.md#^break-statement) statement is encountered.
