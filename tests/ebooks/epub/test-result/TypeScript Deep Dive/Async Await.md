...
You don't have to understand this, but it's fairly simple if you've [read up on generators](Generators.md). The function `foo` can be simply wrapped up as follows:
...
**Async - Await** has been supported by [TypeScript since version 1.7](https://www.typescriptlang.org/docs/handbook/release-notes/typescript-1-7.html). Asynchronous functions are prefixed with the _async_ keyword; _await_ suspends the execution until an asynchronous function return promise is fulfilled and unwraps the value from the _Promise_ returned. It was only supported for **target es6** transpiling directly to **ES6 generators**.
